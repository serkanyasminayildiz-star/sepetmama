...
/**
 * Ödeme mutabakatı: PENDING siparişleri iyzico'ya sorup gerçekte ödenip
 * ödenmediğini bulur.
 *
 ...
 * çekilmiş olmasına rağmen sipariş PENDING'de kalır ve kimse fark etmez.
 *
 * Varsayılan: yalnızca RAPOR (hiçbir şey değiştirilmez).
 * `?uygula=1` verilirse ödendiği doğrulananlar CONFIRMED'e çekilir; stok ve
 * kupon sayacı callback'teki aynı desende, tek transaction içinde işlenir.
 */
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({
    mod: uygula ? 'UYGULA' : 'SADECE_RAPOR',
